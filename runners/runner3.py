...
from .attn_vis import visualize_grid_to_grid_with_cls
...
class NCSNRunner():
    ...
    def sample(self):
        ...
        if not self.config.sampling.fid:
            ...
            for i in tqdm(range(n_rounds), desc='Generating samples for FID'):
                ...
                # visualize_grid_to_grid_with_cls(attention_maps[4][0,4,:,:], 105, all_samples[0])
                # exit(0)
    ...
    def video_gen(self, scorenet=None, ckpt=None, train=False):
        ...
        for i, (x0, real_, _) in tqdm(enumerate(dataloader), total=min(max_data_iter, len(dataloader)), desc="\nvideo_gen dataloader"):
            ...
            for i_frame in tqdm(range(n_iter_frames), desc="Generating video frames"):
                ...
                cache = get_local.cache
                print(list(cache.keys()))
                attention_maps = cache['favor_attention']
                # print(len(attention_maps[0]))
                # print(attention_maps[0].shape)

                # print(x0[0].shape)
                for i in range(288):
                    visualize_grid_to_grid_with_cls(attention_maps[1][0,0,:,:], i, x0[0],32)
                # exit(0)
    ...
    def test_speed(self, scorenet=None, train=False):
        ...
        flops_dict,_ = flop_count(scorenet,(x,t,x0,cond))
        flops = FlopCountAnalysis(scorenet,(x,t,x0,cond))

        for i in range(10):
            _=scorenet(x,t,x0,cond)
        ...
        bs,ch,h=1,self.config.model.ngf,all_resolutions[1]
    ...
    def gen_attn_map(self,scorenet=None, train=False):
        if train:
            assert(scorenet is not None and ckpt is not None)
            max_data_iter = 1   # self.config.sampling.max_data_iter
            preds_per_test = 1  # self.config.sampling.preds_per_test
