...
def cls_padding(image, mask, cls_weight, grid_size):
    ...
    H, W = image.shape[:2]
    delta_H = max(1,int(H/grid_size[0]))
    delta_W = max(1,int(W/grid_size[1]))
    ...
    padded_image = Image.fromarray(padded_image.transpose(1,2,0))
...
def visualize_grid_to_grid_with_cls(att_map, grid_index, image, grid_size=14, alpha=0.6):
    ...
    mask = attention_map[0:].reshape(grid_size[0], grid_size[1])
    mask = Image.fromarray(mask).resize((image.shape[1:]))
    
    # padded_image ,padded_mask = cls_padding(image, mask, cls_weight, grid_size)
    ...
    image = image.cpu().numpy()
    image = Image.fromarray(((image-image.min())/image.max()*255).astype(np.uint8).transpose(1,2,0))
    image.save('vis.png')
    mask.convert('RGB').save(f'mask/mask-{grid_index}.png')
    
    img = Image.blend(image, mask.convert('RGB'), 0.5) # blend_img = img1 * (1 – 0.3) + img2* alpha
    img.save(f"mask/blend-{grid_index}.png")
