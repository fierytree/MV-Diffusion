...
export data="/data/dengzijun/data/smnist/"
export nfp="10"

export exp="/data/dengzijun/dengzijun/ijcai23/smmnist_big5_spynet_cross2/"
...
# export exp="../output/ijcai23/smmnist_interp_big_c5t5f5_SPADE_spynet_cross2"
...
# cp ${exp}/code/runners/ncsn_runner.py runners/ncsn_runner.py


CUDA_VISIBLE_DEVICES=8 python main.py --config configs/${config}.yml --data_path ${data} --exp ${exp} --ni --config_mod ${config_mod} sampling.num_frames_pred=${nfp} sampling.preds_per_test=10 sampling.subsample=${steps} model.version=${version} --ckpt ${ckpt} --video_gen -v videos_${ckpt}_${version}_${steps}_nfp_${nfp}_3
